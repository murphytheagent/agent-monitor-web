...
- `/showcase/` - viewer-first landing page for polished browser surfaces. It now leads with a live tokenizer embed, keeps the `Res Publica` architecture illustration visible on the page, and avoids the old route-inventory / curator copy.
...
- `showcase/` — showcase landing page for hosted project surfaces. It keeps the copy sparse, uses a working tokenizer slice from `/tokenizers/` as the main visual, and keeps the `Res Publica` architecture preview visible as a linked secondary exhibit rather than hiding it behind `showcase/res-publica/`.
